...
int main(void)
{
  ...
  // Read from MPU_6050
  uint8_t who = 0;
  HAL_I2C_Mem_Read(&hi2c1, 0x68 << 1, 0x75, I2C_MEMADD_SIZE_8BIT, &who, 1, 100);
  char idbuf[40];
  snprintf(idbuf, sizeof(idbuf), "WHO_AM_I: 0x%02X\r\n", who);
  HAL_UART_Transmit(&huart2, (uint8_t*)idbuf, strlen(idbuf), 100);
  /* USER CODE END 2 */
